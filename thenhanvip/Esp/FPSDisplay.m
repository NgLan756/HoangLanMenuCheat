...
CGRect frame = CGRectMake([UIScreen mainScreen].bounds.size.width / 4 - 4, [UIScreen mainScreen].bounds.size.height - 330, [UIScreen mainScreen].bounds.size.width / 2  + 10, 16);
...
double batLeft = (float)[myDevice batteryLevel] * 60;


    NSMutableString *mustr = [[NSMutableString alloc] init];
    [mustr appendFormat:@"%@",self.getSystemDate];
    self.displayLabel.font = [UIFont fontWithName:@"AvenirNext-HeavyItalic" size:16];


NSString *WMText = [NSString stringWithFormat:@"Thế Nhân ᥫᩣ"];//fps 
